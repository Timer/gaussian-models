...
scggm_theta scggm_refit_step(SMatrix cx, SMatrix cy, scggm_theta &z_theta,
                             int max_iterations, double tolerance, bool verbose,
                             double eta, scggm_theta &Theta0) {
  // TODO
  /*
  function [Theta, obj] = scggm_refit_step( cx, cy, z_theta, maxiter, tol,
  verbose, eta, Theta0)
  Sx	= cx'*cx;
  Sy 	= cy'*cy;
  Sxy 	= cx'*cy;
  ...
  if init_flag == 1 && verbose
        fprintf('sCGGM: error! refit initial Theta_yy not positive
  definite!\n');
  end
  ...
  grady.yy(:)'*(xk1_y.yy(:));
                        ...
                        RHS = lfxk1_y +  L/2 * (sum(diffxk1.xy(:).^2) +
  sum(diffxk1.yy(:).^2));
                        LHS = fxk1;
                        if ( LHS <= RHS + tol)
                                xk = xk1;
                ...
                if ( ik > ls_maxiter )
                        if verbose
                                fprintf( 'sCGGM: refit line search not
  converging,ik = %d\n',ik);
                        end
                        bconv = 0;
                        iter  = max(1, iter - 1);
