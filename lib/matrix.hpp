...
#if ACCELERATE_MODE == ACCELERATE_MODE_CUDA
void checkCuError(cudaError_t res, char const *msg) {
  if (res != cudaSuccess) {
...
#pragma omp parallel for if (parallel_enabled) collapse(2)
  ...
  SMatrix multiply(const SMatrix B, const bool tranA, const bool tranB) {
    ...
    if (shouldAccelerate(false)) {
      ...
      cudaDeviceSynchronize();
      cublasDgeam(cu_handle, CUBLAS_OP_T, CUBLAS_OP_N, M, N, &alpha, C_accelerate_data, N, &beta, C_accelerate_data, M, C_accelerate_data_tran, M);
      cudaDeviceSynchronize();
